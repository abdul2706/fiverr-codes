...
class Homework1Test(unittest.TestCase):
    ...
    def test_create_gaussian_kernel(self):
        ...
        self.assertAlmostEqual(result.sum(), 1.0, places=4, msg='The kernel sums to 1')
        difference = np.linalg.norm(result - expected_result)
        self.assertAlmostEqual(difference, 0.0, places=4, msg='The values for the kernel were not properly computed.')
    ...
    def test_convolve_pixel(self):
        ...
        self.assertEqual(resultOutOfBound, 9, msg='Input pixel was not returned when the kernel was on the border.')
        result = hw1.convolve_pixel(image, kernel, 1, 1)
        self.assertAlmostEqual(result, 18.6, places=4, msg='The correct result is computed.')
    ...
    def test_convolve(self):
        ...
        difference = np.linalg.norm(result - expected_result)
        self.assertAlmostEqual(difference, 0.0, msg='The values for the image were not properly computed.')
    ...
    def test_split_image(self):
        ...
        self.assertAlmostEqual(rDiff, 0.0, msg='The first channel pixel values were not properly computed.')
        self.assertAlmostEqual(gDiff, 0.0, msg='The second channel pixel values were not properly computed.')
        self.assertAlmostEqual(bDiff, 0.0, msg='The third channel pixel values were not properly computed.')
    ...
    def test_merge_image(self):
        ...
        # ensure the proper values are returned
        self.assertAlmostEqual(result[:, :, 0].mean(), 0.0, msg='The first channel pixel values were not properly computed.')
        self.assertAlmostEqual(result[:, :, 1].mean(), 1.0, msg='The second channel pixel values were not properly computed.')
        self.assertAlmostEqual(result[:, :, 2].mean(), 2.0, msg='The third channel pixel values were not properly computed.')
