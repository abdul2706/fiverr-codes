...
def filter_dataset(X, y, c):
    filtered_indices = []
    y_unique = np.unique(y)
    print('[filter_dataset][y_unique]', len(y_unique))
    for val in y_unique:
        indices = np.where(y == val)[0]
        count = len(indices)
        if count >= c:
            filtered_indices.extend(indices)
    print('[filter_dataset][filtered_indices]', len(filtered_indices))
